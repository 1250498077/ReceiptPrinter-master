...
function train(allLayerData, all_y_trues) {
  ...
  for (let z = 0; z < allLayerData.length; z++) {
    ...
    for (let i = neuronNetworkNeuronInstance.length - 1; i >= 0; i--) {
      ...
      for (let j = 0; j < currentSingleLayer.length; j++) {
        ...
        // console.log("currentSingleNeuron.h", currentSingleNeuron.h)
  ...
  for (let i = 0; i < neuronNetworkNeuronInstance.length; i++) {
    for (let j = 0; j < neuronNetworkNeuronInstance[i].length; j++) {
      // console.log("前  前 22222neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))

      let data = createTree(i, j, neuronNetworkNeuronInstance)
      // console.log("中  中 22222neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))

      // console.log("22222neuronNetworkNeuronInstance data", JSON.parse(JSON.stringify(data))
      // console.log('data', JSON.parse(JSON.stringify(data)));
      calculate(data);
      console.log("后  后 22222neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))
      // console.log("前 前 前 neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))
  ...
  function createTree(currentLayerNum, currentNeuronNum, neuronNetworkNeuronInstance) {
    ...
    // console.log("[firstLayer", JSON.parse(JSON.stringify(firstLayer)))
    // console.log("[firstLayer, ...other", JSON.parse(JSON.stringify(otherLayer)))
    // console.log("[firstLayer, ...otherLayer]", JSON.parse(JSON.stringify([firstLayer, ...otherLayer])))
    //console.log("前22222neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))
    let arr = generateCombinations([firstLayer, ...otherLayer]);
    //console.log("后22222neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))


    return arr;
  }

  // console.log("前 前 neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))
  function calculate(allCombineData) {
    ...
    for (let i = 0; i < allCombineData.length; i++) {
      ...
      // console.log("different_sum_in_different_w", JSON.parse(JSON.stringify(different_sum_in_different_w)))
      ...
      for (let j = 0; j < firstNeuron.w.length; j++) {
        ...
        for (let z = 0; z < currentCombineData.length; z++) {
          ...
          if (currentCombineData[z].type === "end") {
            ...
            // console.log("后 different_sum_in_different_w", JSON.parse(JSON.stringify(different_sum_in_different_w)))
            // b只需要push一次
            if (j === 0) {
              // 假设第一层没有b
    ...
    // console.log("前 neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))
    let sum_w_arr = [];
    for (let j = 0; j < firstNeuron.w.length; j++) {
      let sum_w = 0;
      ...
      // console.log("sum_w_arr", sum_w_arr)
      // console.log("sum_w_arrsum_w_arr", sum_w_arr[j])
      ...
      // console.log("neuronNetworkNeuronInstance[firstNeuron.layerNum][firstNeuron.inLayerNum]", neuronNetworkNeuronInstance[firstNeuron.layerNum][firstNeuron.inLayerNum])
    }
    // console.log("后 neuronNetworkNeuronInstance", JSON.parse(JSON.stringify(neuronNetworkNeuronInstance)))
    let sum_b = 0;
    for (let i = 0; i < different_sum_in_different_b.length; i++) {
      sum_b = sum_b + different_sum_in_different_b[i];
    }
    // console.log("sum_b / different_sum_in_different_b.length", different_sum_in_different_b.length)
